...
void grep_main(struct flags *_flags, int argc, char **argv, char *temp) {
  ...
  if (!_flags->is_f && !_flags->is_e)
    snprintf(patternL, buff_size, argv[optind++]);
  if (_flags->is_f)
    cols = save_patt_file(patternL, temp);
  if (!_flags->is_f && _flags->is_e)
    snprintf(patternL, buff_size, temp);

  if (cols != -1) {
    int file_c = (argc - optind > 1) ? 1 : 0;

    for (int i = optind; i < argc; i++) {
      if (file_c && !_flags->is_h && !_flags->is_l)
        printf("%s:", argv[i]);
...
void filework(struct flags *_flags, FILE *fp, regex_t reg, char *file) {
  ...
  while (fgets(text, buff_size - 1, fp) != NULL) {
    ...
    if (status == 0 && !_flags->is_v)
      match = 1;
    if (status == REG_NOMATCH && _flags->is_v)
      match = 1;
  ...
  if (_flags->is_l && line_matches > 0)
    printf("%s\n", file);
  if (_flags->is_c && !_flags->is_l)
    printf("%d\n", line_matches);
}
